...
class CacheService:
    ...
    def get_cached_weather_data(self, location_id: int):
        ...
        try:
            ...
            print(
                f"error getting cache of snapshot for city: {location_id}, {exception}"
            )
            return None

    def set_cached_weather_data(self, location_id: int, weather_data: dict):
        """
        ...
        try:
            ...
            # NOTE: in production this should be logged
            print(
                f"error setting cache of snapshot for city: {location_id}, {exception}"
            )
...
class ApiCallService:
    ...
    def find_city_data(self, **kwargs) -> Optional[tuple[str, str, float, float]]:
        """
        ...
        url = f"https://maps.googleapis.com/maps/api/geocode/json?{params}&key={self.geocoding_api_key}&result_type=locality&language=en"
